...
def get_bot_session(telegram_user_id: str) -> dict | None:
    ...
    with _BOT_SESSION_LOCK:
        ...
        if cached and cached.get("dirty") and cached.get("cache_until", now) > now:
...
def list_territories_for_map(
    ...
    if occupied_only:
        rows = [row for row in rows if row.get("is_locked") or row.get("has_agent")]
    return rows
